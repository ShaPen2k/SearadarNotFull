...
/**
 * Этот класс является частью системы обработки сообщений с морских радаров.
 * Класс-конвертер Mr231Converter реализует интерфейс SearadarExchangeConverter.
 * Он отвечает за преобразование входящих сообщений в объекты SearadarStationMessage на основе разных типов сообщений.
 * <p>
 * Класс Mr231Converter содержит методы для разбора входящего сообщения, извлечения актуальной информации и создания
 * соответствующих объектов SearadarStationMessage, таких как TrackedTargetMessage, WaterSpeedHeadingMessage и RadarSystemDataMessage.
 * <p>
 * Класс имеет возможность обрабатывать типы сообщений "TTM", "VHW" и "RSD" и конвертировать их соответствующим образом.
 * Он считывает сообщение, определяет тип сообщения, а затем вызывает соответствующие методы для генерации правильного объекта.
 * Если сообщение имеет тип "RSD", выполняется дополнительная проверка для проверки правильности объекта RadarSystemDataMessage.
 *
 */
